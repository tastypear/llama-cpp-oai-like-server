...
parser.add_argument("--chat-format", type=str, help="use chat fomart config. It will override the arguments.", default="")
...
if args.chat_format != "":
    import toml
    try:
        with open("chat_format.toml", "r", encoding="utf-8") as f:
            chat_format = toml.load(f)[args.chat_format]
    except FileNotFoundError:
        raise Exception("The file 'chat_format.toml' does not exist.")
    except toml.TomlDecodeError:
        raise Exception("There was an error decoding the TOML file.")
...
def convert_chat_with_template(messages):
    prompt = ""
    for line in messages:
        if (line["role"] == "system"):
            prompt += chat_format["system"].format(content = line['content'])
        if (line["role"] == "user"):
...
def convert_chat(messages):
    if chat_format is not None:
        return convert_chat_with_template(messages)
